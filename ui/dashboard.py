...
class Ui_AdminDashboard(object):
    def setupUi(self, AdminDashboard):
        ...
        # === Stock Page UI with Tabs(Retail) ===
        ...
        self.wholesale_filter_input.setPlaceholderText("Filter by item namey...")
        ...
        self.tableCheckoutCart.setMinimumHeight(250)  # Increased slightly from 200
        ...
        def _lcd_block(title, obj_name):
            ...
            w_layout = QtWidgets.QVBoxLayout(
                w
            )  # Changed back to vertical for label on top
        ...
        below_h.addStretch(1)  # Add space to push content rightward

        center_col.addLayout(below_h, stretch=0)  # No stretch to keep at bottom
        ...
        damage_layout = QtWidgets.QVBoxLayout(self.page_damage)
        ...
        self.filter_input_damage.setFixedWidth(300)  # Fixed width for filter input
        filter_container.addWidget(self.filter_input_damage)
        filter_container.addStretch()

        damage_layout.addLayout(filter_container)
        damage_layout.addSpacing(2)  # Reduced space between filter and table to 2

        # === Table and LCDs Section ===
        table_lcds_h = QtWidgets.QHBoxLayout()
        table_lcds_h.setSpacing(2)  # Reduced space between table and LCDs to 2
        ...
        self.table_damage.setEditTriggers(QtWidgets.QAbstractItemView.DoubleClicked)
        ...
        self.table_damage.setFixedWidth(
            int(950 * 0.95)
        )  # Extended table width to reduce gap to LCDs
        ...
        table_lcds_h.addWidget(
            self.table_damage, stretch=9
        )  # Increased stretch to extend table further
